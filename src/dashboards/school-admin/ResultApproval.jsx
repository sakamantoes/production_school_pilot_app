...
const RemarkModal = ({
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={`px-6 py-4 ${isReject ? "bg-gradient-to-r from-rose-50 to-red-50" : "bg-gradient-to-r from-emerald-50 to-green-50"} border-b`}
        >
      ...
      <div className="p-5">
        ...
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-blue-500 to-blue-600 flex items-center justify-center text-white font-bold text-sm shadow-sm">
              {(result.studentName || result.student?.user?.firstName)?.charAt(
                0,
              ) || "S"}
            </div>
            <div>
        ...
        <div className="grid grid-cols-2 gap-3 mb-4 p-3 bg-slate-50 rounded-xl">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-sm font-semibold text-slate-700">
                {result.enrollment?.class?.name || result.className}{" "}
                {result.enrollment?.arm?.name || result.armName}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 mb-4">
          <div className="text-center">
            ...
            <div
              className={`inline-flex items-center justify-center px-4 py-2 rounded-xl font-bold text-lg ${getScoreBg(result.examScore)} ${getScoreColor(result.examScore)}`}
            >
              {result.examScore}%
            </div>
          </div>
          <div className="text-center">
            ...
            <div
              className={`inline-flex items-center justify-center px-4 py-2 rounded-xl font-bold text-lg ${getScoreBg(result.totalScore)} ${getScoreColor(result.totalScore)}`}
            >
              {result.totalScore}%
            </div>
          </div>
          ...
          <div className="mt-3 pt-3 border-t border-slate-100">
            <div className="flex items-center gap-2 text-xs text-slate-500">
              ...
              <span>
                ...
                {result.approvedByUser?.firstName ||
                  result.approvedBy ||
                  "Admin"}
              </span>
        ...
        <div className="flex items-start justify-between flex-wrap gap-2 mb-2">
          <div>
            <p className="font-semibold text-slate-800">
              {item.result?.student?.user?.firstName}{" "}
              {item.result?.student?.user?.lastName}
            </p>
            <p className="text-sm text-slate-500">
              {item.result?.subject?.name} •{" "}
              {item.result?.enrollment?.class?.name}{" "}
              {item.result?.enrollment?.arm?.name}
            </p>
          </div>
          <div className="flex items-center gap-2">
            <span
...
const ResultApproval = () => {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const [classRes, pendingRes, statsRes, historyRes] =
      ...
      setClasses(
        toArray(
          classRes.status === "fulfilled" ? classRes.value : null,
          "classes",
        ),
      );
      setPendingClasses(
        toArray(
          pendingRes.status === "fulfilled" ? pendingRes.value : null,
          "classes",
          "data",
        ),
      );
  ...
  const fetchResults = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await resultApprovalAPI.getResultsByClass(
        selectedClass,
        selectedArm,
        statusFilter,
      );
  ...
  const handleSingleActionConfirm = async (remark) => {
    ...
    try {
      ...
      toast.error(
        error?.response?.data?.message || `Failed to ${remarkModal.action}`,
      );
    } finally {
      setSubmitting(false);
    }
  ...
  const handleBulkActionConfirm = async (remark) => {
    ...
    try {
      ...
      toast.error(
        error?.response?.data?.message || `Bulk ${remarkModal.action} failed`,
      );
    } finally {
      setSubmitting(false);
    }
            ...
            <div>
              ...
              <div className="h-2 bg-slate-100 rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${(stats.approved / stats.total) * 100 || 0}%`,
                  }}
                />
              </div>
            </div>
      ...
      {approvalHistory.length === 0 ? (
        <div className="text-center py-16 bg-white rounded-2xl border border-slate-200">
          ...
          <p className="text-sm text-slate-400">
            Results will appear here once reviewed
          </p>
        </div>
      ) : (
        <>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
      {/* Header */}
      <div
        className="relative overflow-hidden px-6 py-8 mb-6"
        style={{
          background:
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              ...
              <p className="text-indigo-200 text-sm mt-1">
                Review, approve, and manage examination results
              </p>
            </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-amber-700">
                  {pendingClasses.length} class(es) have results waiting for
                  your review
                </p>
